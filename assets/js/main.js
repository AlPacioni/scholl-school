(() => {
  ...
  const setLang = (lang) => {
    ...
    if (output) {
      output.textContent = `Lokale Aufrufe auf diesem Gerät: ${count}`;
    }
    localStorage.setItem('bh_lang', lang);
  };

  const savedLang = localStorage.getItem('bh_lang') || 'de';
  setLang(savedLang);
  ...
  if (lightbox) {
    ...
    const openLightbox = (img) => {
      ...
      lightboxImg.alt = img.alt || 'Erweitertes historisches Bild';
